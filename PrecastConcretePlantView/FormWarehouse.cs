...
namespace PrecastConcretePlantView
{
    public partial class FormWarehouse : Form
    {
        ...
        private void FormWarehouse_Load(object sender, EventArgs e)
        {
            if (id.HasValue)
            {
                try
                {
                    ...
                    if (view != null)
                    {
                        ...
                        textBoxWMName.Text = view.WarehouseManagerFullName;
                        warehouseComponents = view.WarehouseComponents;
                        if (warehouseComponents != null)
                        {
                            dataGridView.Rows.Clear();
                            foreach (var pc in warehouseComponents) dataGridView.Rows.Add(new object[] { pc.Key, pc.Value.Item1, pc.Value.Item2 });
                        }
                    }
                }
                catch (Exception ex)
            ...
            else warehouseComponents = new Dictionary<int, (string, int)>();
        }
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(textBoxWMName.Text))
            {
            ...
            try
            {
                logic.CreateOrUpdate(new WarehouseBindingModel
                {
                    ...
                    WarehouseManagerFullName = textBoxWMName.Text,
