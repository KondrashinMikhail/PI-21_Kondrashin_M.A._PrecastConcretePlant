...
namespace PrecastConcretePlantBusinessLogic.MailWorker
{
    public abstract class AbstractMailWorker
    {
        ...
        public AbstractMailWorker(IMessageInfoLogic messasgeInfoLogic) => _messageInfoLogic = messasgeInfoLogic;
        ...
        {
            if (string.IsNullOrEmpty(_mailLogin) || string.IsNullOrEmpty(_mailPassword)) return;
            if (string.IsNullOrEmpty(_popHost) || _popPort == 0) return;
            if (_messageInfoLogic == null) return;
            var list = await ReceiveMailAsync();
            foreach (var mail in list) _messageInfoLogic.CreateOrUpdate(mail);
        }
