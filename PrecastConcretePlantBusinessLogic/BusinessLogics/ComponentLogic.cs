...
namespace PrecastConcretePlantBusinessLogic.BusinessLogics
{
    public class ComponentLogic : IComponentLogic
    {
        ...
        public ComponentLogic(IComponentStorage componentStorage) => _componentStorage = componentStorage;
        public List<ComponentViewModel> Read(ComponentBindingModel model)
        {
            if (model == null) return _componentStorage.GetFullList();
            if (model.Id.HasValue) return new List<ComponentViewModel> { _componentStorage.GetElement(model) };
        ...
        public void CreateOrUpdate(ComponentBindingModel model)
        {
            ...
            if (model.Id.HasValue) _componentStorage.Update(model);
            else _componentStorage.Insert(model);
        }
